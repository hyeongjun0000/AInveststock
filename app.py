...
def detect_market(ticker):
    """
    반환: (is_korean, market_type, currency_sym, flag_emoji)
    """
    upper = ticker.upper()
...
def ai_levels(df, price):
    ...
    return dict(
        ...
        short_entry = round(price * 0.997, 2),           # ★ 숏 진입가 (현재보다 살짝 낮게)
        short_cover = round(price - float(atr) * 2.0, 2), # ★ 숏 커버가 (하락 목표)
...
is_korean, market_type, ccy, flag = detect_market(ticker_input) if ticker_input else (False, "", "$", "🇺🇸")
...
if run and ticker_input:
    with st.spinner("📡 데이터 수집 중 ..."):
        try:
            ...
            if is_korean:
                # 한국 주식: 가격이 이미 KRW
                seed_trade = seed_krw          # 원화 시드 그대로
                price_krw  = price
            else:
                # 미국 주식: USD → KRW 환산
                seed_trade = seed_krw / usd_krw  # USD 시드
            ...
            mkt_w = get_hist(mkt_ticker_w, "3y",  "1wk")
            mkt_m = get_hist(mkt_ticker_m, "5y",  "1mo")
            spy_w = get_hist("^KS11" if is_korean else "SPY", "3y", "1wk")

            spy_k,  spy_d  = calc_stoch(spy_w)
            mktw_k, mktw_d = calc_stoch(mkt_w)
            mktm_k, mktm_d = calc_stoch(mkt_m)
            ...
            sig_key, sig_score, sig_prob, sig_details = swing_signal(
                ...
                0.6, 0.1   # 임시, 아래서 애널리스트 후 재계산
            )
            is_short_signal = sig_key in ("strong_short", "short")

            # ── AI 매매전략: 롱/숏 분기 ★핵심 수정★ ─────────────────────
            if is_short_signal:
                # 숏 전략: 진입가(높게) → 커버가(낮게)
                entry = lv["short_entry"]   # 현재가보다 살짝 낮은 숏 진입
                cover = lv["short_cover"]   # ATR×2 아래 커버(익절)
                ...
                buy_color  = "#4a9eff"   # 숏 진입 = 파란색
                sell_color = "#30d158"   # 커버(이익) = 초록
                qty_label  = "수량 / 진입총액"
            else:
                # 롱 전략: 매수가 → 매도가
                entry = lv["buy"]
            ...
            st.markdown(stoch_html(spy_label, spy_k, spy_d), unsafe_allow_html=True)
            st.markdown(stoch_html(mkt_label_w, mktw_k, mktw_d), unsafe_allow_html=True)
            st.markdown(stoch_html(mkt_label_m, mktm_k, mktm_d), unsafe_allow_html=True)
            ...
            # 숏 신호일 때 배너
            if is_short_signal:
                short_clr = SIGNAL_META[sig_key][1]
                st.markdown(f"""
            ...
            st.markdown(pct_bar_html("중기 목표", "📅", lv["mid_t"], mid_tp, "이 가격을 돌파하면 추가 상승이 기대돼요", ccy), unsafe_allow_html=True)
            st.markdown(pct_bar_html("장기 목표", "📅", lv["lng_t"], lng_tp, "상승 추세가 강할 때 노릴 수 있어요", ccy), unsafe_allow_html=True)
            st.markdown(pct_bar_html("중기 방어선", "🛡️", lv["mid_d"], mid_dp, "이 가격 아래로 내려가면 추가 하락 위험이 있어요", ccy, False), unsafe_allow_html=True)
            st.markdown(pct_bar_html("장기 방어선", "🔒", lv["lng_d"], lng_dp, "핵심 지지 구간 · 이탈 시 추세 전환 가능성", ccy, False), unsafe_allow_html=True)
